...
export function decryptSymmetric(
  ct: ArgumentsType<SubtleCrypto['decrypt']>[2],
  key: CryptoKey,
  iv: AesGcmParams['iv'],
  additionalData: AesGcmParams['additionalData']
) {
...
export class Cryptor {
  ...
  public async toJSON(): Promise<ICryptorKeyPair> {
    ...
    const b64Key: IBase64JSONCryptoKey = {
      ...
      additionalData: uInt8ArrayToB64(privateKeyEncrypted.additionalData),
    }
  ...
  public async fromJSON(json: ICryptorKeyPair, passphrase: string | Uint8Array, salt: string | Uint8Array) {
    ...
    this.authBits = bits.slice(32, 32)
    const { privateKeyEncrypted } = json
